...
export const apiDefinitions = pgTable("api_definitions", {
  ...
  created_by: integer().references(() => users.id),                                                 // 创建用户
  created_at: timestamp({ withTimezone: true }).notNull().defaultNow(),                             // 创建时间
  updated_by: integer().references(() => users.id),                                                 // 更新用户
  updated_at: timestamp({ withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()), // 更新时间
});
