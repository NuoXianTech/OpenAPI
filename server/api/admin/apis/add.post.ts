...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const created = await apiService.addApi(admin.id || null, {
    ...
    version: body.version?.toString().trim() || 'v1',
    tags: body.tags?.toString().trim() || null,
    authType: body.authType?.toString().trim() || 'none',
    requestSchema: body.requestSchema?.toString().trim() || null,
    responseSchema: body.responseSchema?.toString().trim() || null,
    requestExample: body.requestExample?.toString().trim() || null,
    responseExample: body.responseExample?.toString().trim() || null,
