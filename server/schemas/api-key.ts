...
const apiKeyNameSchema = z.string().trim().max(80, maxMessage('名称', 80))
...
const apiKeyTotalQuotaSchema = z.preprocess(
  (v) => {
...
const apiKeyCreateCountSchema = z.coerce.number().int().min(1, minMessage('生成数量', 1, '个')).max(5, maxMessage('生成数量', 5, '个')).default(1)

function nullableArraySchema<T extends z.ZodTypeAny>(item: T, max: number) {
  return z.preprocess(
    (v) => {
      if (v === undefined) return undefined
