 ...
 * 4. 扫描 `server/api-capabilities/v{N}/<code>.ts`，生成 `#api-capabilities`
 * 5. 违反路由或能力声明约定时在构建期抛错
 *
...
const CAPABILITY_FILE_RE = /^([a-z0-9][a-z0-9-]*)\.(ts|mts|js|mjs)$/
...
async function buildCapabilitySources(
  rootDir: string,
  apis: ManifestApi[]
): Promise<CapabilitySource[]> {
  const capabilityRoot = join(rootDir, 'server', 'api-capabilities')
  if (!existsSync(capabilityRoot)) return []

  const manifestKeys = new Set(apis.map(api => `${api.pathVersion}:${api.code}`))
  const sources: CapabilitySource[] = []
  const seenKeys = new Set<string>()
  const versionEntries = await readdir(capabilityRoot, { withFileTypes: true })

  for (const versionEntry of versionEntries) {
    if (!versionEntry.isDirectory() || !VERSION_DIR_RE.test(versionEntry.name)) continue
    const versionDir = join(capabilityRoot, versionEntry.name)
    const entries = await readdir(versionDir, { withFileTypes: true })

    for (const entry of entries) {
      if (!entry.isFile() || entry.name.endsWith('.d.ts')) continue
      const match = CAPABILITY_FILE_RE.exec(entry.name)
      if (!match) {
        throw new Error(
          `[api-capabilities] 文件名不合法：server/api-capabilities/${versionEntry.name}/${entry.name}；`
          + `必须使用 <code>.ts 命名。`
        )
      }

      const code = match[1]!
      const key = `${versionEntry.name}:${code}`
      if (!manifestKeys.has(key)) {
        throw new Error(
          `[api-capabilities] ${versionEntry.name}/${code} 没有对应的公共接口路由；`
          + `请先创建 server/routes/${versionEntry.name}/${code}。`
        )
      }
      if (seenKeys.has(key)) {
        throw new Error(`[api-capabilities] 重复声明：${versionEntry.name}/${code}`)
      }
      seenKeys.add(key)

      const fullPath = join(versionDir, entry.name)
      sources.push({
        pathVersion: versionEntry.name,
        code,
        importPath: `~~/${relative(rootDir, fullPath).split(sep).join('/')}`
      })
    }
  }
...
function renderCapabilityModule(sources: CapabilitySource[]): string {
  const imports = sources.map((source, index) => (
    `import { apiCapabilityDefinition as definition${index} } from ${JSON.stringify(source.importPath)}`
  ))
  const definitions = sources.map((source, index) => (
    `{ pathVersion: ${JSON.stringify(source.pathVersion)}, code: ${JSON.stringify(source.code)}, definition: definition${index} }`
  )).join(', ')
  return [
    '// AUTO-GENERATED by modules/api-manifest.ts — DO NOT EDIT',
    ...imports,
    `export const API_CAPABILITY_DEFINITIONS = [${definitions}]`,
    ''
  ].join('\n')
}
...
export default defineNuxtModule({
  ...
  setup(_options, nuxt) {
    ...
    nuxt.hook('nitro:config', async (nitroConfig) => {
      ...
      const capabilitySources = await buildCapabilitySources(rootDir, initialManifest)
      nitroConfig.virtual['#api-capabilities'] = renderCapabilityModule(capabilitySources)
    ...
    addTypeTemplate({
      filename: 'types/api-capabilities.d.ts',
      getContents: () => [
        `declare module '#api-capabilities' {`,
        `  import type { ApiCapabilityManifestEntry } from '${relative(join(rootDir, '.nuxt', 'types'), join(rootDir, 'shared', 'types', 'api-capability')).split(sep).join('/')}'`,
        `  export const API_CAPABILITY_DEFINITIONS: ApiCapabilityManifestEntry[]`,
        `}`,
        ''
      ].join('\n')
    }, { nitro: true, nuxt: true })
