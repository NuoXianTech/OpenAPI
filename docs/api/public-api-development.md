...
1. 在 `/admin/apis/upstreams` 创建 `external` Upstream。
2. 配置 HTTPS Target；Platform 会执行 SSRF 和地址安全校验。
3. 在接口目录点击“自定义接口”，设置公开路径、上游路径模板、API Key、积分、限流和统计。
4. 保存后 Platform 自动创建 Route 并激活新的 Routing Revision。

External Upstream 不要求实现 Service 发现或配置协议，Platform 也不会管理其内部业务配置。
...
7. 在 Platform 创建 Internal Upstream，填写 Target 和独立 Service Token。
8. 执行“发现 Service”，查看 OpenAPI Endpoint 和配置 Schema。
9. 在接口目录审查 Endpoint 并点击发布。Platform 默认沿用 Service Path 作为公开 Path，自动创建或复用 Product、Version、Route，并激活新的 Routing Revision。

Service OpenAPI 变化不会在未经确认时暴露新 Endpoint，也不会静默改写现有公开 Route。新增 Endpoint 会显示为“可发布”，管理员明确点击发布后才生效。
...
1. 先在 Platform 接口目录停用公开接口；Platform 会自动发布不再包含该 Route 的 Revision。
