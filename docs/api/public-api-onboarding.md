...
- gate 层错误码（`MISSING_API_KEY` / `RATE_LIMITED` / `API_NOT_REGISTERED` …）登记在 [server/config/api-guard.ts](../../server/config/api-guard.ts#L28) 的 `API_GUARD_ERROR`；业务 handler 自己的 `code`（`ALGORITHM_NOT_FOUND` / `UPSTREAM_ERROR` …）SCREAMING_SNAKE_CASE 内联即可，不必登记。
