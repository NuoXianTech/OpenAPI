...
- Route 与治理变更自动生成 Routing Revision，并可通过发布历史回滚。
...
1. 在接口目录修改 API Key、统计或积分，确认 Platform 自动生成新发布版本且流量立即使用新配置。
2. 使用高级设置制造路径冲突，确认 Route 期望配置被保存、接口显示“待应用”，但活动流量继续使用旧发布版本。
3. 解决冲突并点击“应用变更”，确认待处理状态消失且新配置生效。
4. 在接口目录停用接口，确认自动生成新发布版本，公开路径返回 `404 API_NOT_FOUND`。
5. 在发布历史中激活停用前的版本，Route 立即恢复。
