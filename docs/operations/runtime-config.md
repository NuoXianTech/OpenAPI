...
管理员账号与普通用户共用 `users` 表，通过 `users.role='admin'` 区分。启动时如果不存在任何管理员账号，服务端会自动创建用户名为 `admin` 的管理员，并将随机密码输出到控制台；之后管理员账号、密码和状态都应在后台用户管理中维护。

发布前必须生成并应用包含 `users.role varchar(20) not null default 'user'` 的数据库迁移；已有 `kind` 字段的环境应由迁移直接改名为 `role`。
