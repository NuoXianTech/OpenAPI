...
- `0.1.0` 的 `0000` 是正式基线。一旦 `0.1.0` 发布，不再修改或重新生成该 SQL、journal 条目和 snapshot。
- 后续每个准备发布的 Schema 变更批次运行一次 `pnpm db:generate`，生成新的 `0001`、`0002` 等增量迁移，并与代码一起审查、测试和发布。不要为同一开发阶段的每个中间编辑分别生成迁移。
- 禁止删除、重命名或改写已经发布的迁移。需要修正时追加新的前向迁移。
...
- `0.1.0` 之前的实验数据库不属于该迁移链，必须使用新数据库或经过演练的数据导入；从正式 `0.1.0` 开始可以连续升级到 `0.1.1` 及后续版本。
...
以 `0.1.0 → 0.1.1` 为例，`0.1.1` 产物包含原有 `0000` 和新增的 `0001`。生产数据库已经记录 `0000`，因此执行新产物时只应用 `0001`。

1. 下载或拉取 `0.1.1` 预构建产物，保留 `0.1.0` 产物，不要原地覆盖。
2. 核对目标数据库连接、新产物版本和迁移是否向后兼容旧应用。
3. PostgreSQL 创建可恢复备份；PGlite 必须先停止旧 Platform，再对完整数据目录创建一致性快照。
4. PGlite、破坏性迁移或未确认兼容性的迁移保持 Platform 停止。只有经过审查的 PostgreSQL 向后兼容迁移才允许旧版本在迁移期间继续服务。
5. 使用新产物显式执行迁移。
6. 迁移成功后才把流量切换到 `0.1.1`；失败时不要启动新版本。旧版本能否继续运行取决于该迁移是否向后兼容。
7. 验证 `/api/ready`、登录、管理后台、活动 Routing Revision 和公开 Route。
