...
> | 文档 | 关注层面 | 何时查 |
> | --- | --- | --- |
> | [restful-api-style.md](./restful-api-style.md) | **风格层**：URL、HTTP 方法、响应壳、状态码、版本 | 设计接口形状时 |
> | [api-conventions.md](./api-conventions.md) | **落地规范**：目录约定、构建期约束、响应工具、计费标记、后台注册字段 | 写 route handler 时 |
...
```
① 设计接口形状        遵循 restful-api-style.md（URL / method / 响应壳 / 状态码 / 版本）
② 选形态              §1 决策表
...
输出层若涉及**多格式 / 字符集 / JSONP**（内容协商型接口），实现分工严格遵循 [api-conventions.md §4.0](./api-conventions.md#40-内容协商型接口encode-多格式输出)：`encode=json`（含默认）走标准响应壳，其余格式各自直出。绝大多数常规接口不需要这一层，只返回 JSON 壳即可。
...
本层的**全部规则**（路径与 `code` 目录约定、构建期 fail-fast 约束、文件名→method 映射、响应壳、入参校验）都在 [api-conventions.md §1–§4](./api-conventions.md) 里，**不在此重复**。接入时只需记住三条硬线：
...
handler 应保持薄——把逻辑委托给 [§3](#3-业务实现层-serverlib-本文重点) 的业务层。最小完整示例见 api-conventions.md §6 与现有的 [server/routes/v1/yiyan/index.get.ts](../server/routes/v1/yiyan/index.get.ts)。
...
详见 [api-conventions.md §5](./api-conventions.md#5-计费标记)。

---

## 6. 重启 + 后台启用（形态 B 必做）

> **核心：注册是自动的，你只需启用。** 字段速查见 [api-conventions.md §7](./api-conventions.md#7-后台启用与配置必做)；下面讲清背后的同步机制。
...
> **`code` / `pathVersion` 是关联键，等于公开契约**：它们必须严格等于目录上的 `v{N}` + 第一层目录名。事后改名 = manifestSync 把旧行标 orphan + 建一个全新行，**丢失既有调用统计、API Key 关联、限流配置**。改名要走升版本流程（[restful-api-style.md §5](./restful-api-style.md#5-版本控制)）。
...
在 [api-conventions.md §9](./api-conventions.md#9-检查清单pr-自查) 之上，补充业务层与流程项：

**通用**

- [ ] 接口形状遵循 [restful-api-style.md](./restful-api-style.md)（URL 名词复数、method 语义、状态码、版本前缀）
