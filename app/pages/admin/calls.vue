  ...
  <div class="auth-shell">
    <div class="auth-panel">
      <div class="auth-card" style="width:min(1180px, 96vw);">
        <div class="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <h1 class="auth-title">调用统计</h1>
            <p class="auth-subtitle">查看调用日志和统计汇总。</p>
          </div>
        ...
        <div class="grid gap-4">
          <section class="grid gap-2">
            ...
            <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              <div v-for="item in stats" :key="item.id" class="p-3 rounded-[12px] border border-border bg-white text-sm">
          ...
          <section class="grid gap-2">
            <h2 class="font-semibold">调用日志</h2>
            <div class="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              <div v-for="item in calls" :key="item.id" class="p-3 rounded-[12px] border border-border bg-white text-sm">
                <div class="font-semibold">API #{{ item.apiId }} · {{ item.method }}</div>
                <div class="text-xs text-muted break-all">{{ item.path }}</div>
                <div class="text-xs text-muted mt-1">状态 {{ item.statusCode }} / 耗时 {{ item.latencyMs }}ms</div>
              </div>
            </div>
          </section>
        </div>
    ...
    <div class="auth-hero">
      <div class="auth-hero-card">
        ...
        <div class="auth-chip">Logs · Stats</div>
      </div>
    </div>
  </div>
</template>
