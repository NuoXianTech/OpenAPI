  ...
  <div class="auth-shell">
    <div class="auth-panel">
      <div class="auth-card" style="width:min(1180px, 96vw);">
        <div class="flex items-center justify-between gap-3 flex-wrap">
          <div>
            <h1 class="auth-title">用户管理</h1>
            <p class="auth-subtitle">编辑用户、封禁用户，并管理该用户的 API Key。</p>
          </div>
        ...
        <div class="grid gap-3 md:grid-cols-[320px_1fr]">
          <div class="grid gap-3">
            <input v-model="keyword" class="auth-input" placeholder="搜索用户名/邮箱">
            <button class="auth-button" @click="loadUsers">搜索</button>
            <div class="grid gap-2 max-h-[520px] overflow-auto pr-1">
              <button v-for="user in users" :key="user.id" class="text-left p-3 rounded-[12px] border border-border bg-white hover:bg-bg" @click="pickUser(user)">
                <div class="font-semibold">{{ user.username }}</div>
                <div class="text-xs text-muted">{{ user.email }}</div>
                <div class="text-xs text-muted">激活: {{ user.isActive ? '是' : '否' }} / 封禁: {{ user.isBanned ? '是' : '否' }}</div>
              </button>
            </div>
          </div>
          ...
          <div class="grid gap-4">
            <div class="grid gap-3 border border-border rounded-[14px] p-4 bg-white">
              ...
              <div class="grid gap-3 md:grid-cols-2">
                <input v-model="form.username" class="auth-input" placeholder="用户名">
                <input v-model="form.email" class="auth-input" placeholder="邮箱">
                <input v-model="form.displayName" class="auth-input" placeholder="显示名称">
                <input v-model="form.avatarUrl" class="auth-input" placeholder="头像 URL">
              </div>
              <div class="grid gap-2 md:grid-cols-2">
                <label class="flex items-center gap-2 text-sm"><input v-model="form.isActive" type="checkbox"> 激活</label>
                <label class="flex items-center gap-2 text-sm"><input v-model="form.isBanned" type="checkbox"> 封禁</label>
              </div>
              <div class="auth-actions">
                <button class="auth-button" @click="saveUser">保存用户</button>
                <button class="auth-button auth-ghost" @click="deleteUser(form.id)">删除用户</button>
                <button class="auth-button auth-ghost" @click="toggleBan(form as UserItem)">切换封禁</button>
              </div>
            </div>

            <div class="grid gap-3 border border-border rounded-[14px] p-4 bg-white">
              <div class="font-semibold">用户 API Key</div>
              <div class="grid gap-2 md:grid-cols-[1fr_auto]">
                <input v-model="keyName" class="auth-input" placeholder="新密钥名称">
                <button class="auth-button" @click="createApiKey">新增密钥</button>
              </div>
              <div class="grid gap-2">
                <div v-for="item in apiKeys" :key="item.id" class="p-3 rounded-[12px] border border-border">
    ...
    <div class="auth-hero">
      <div class="auth-hero-card">
        ...
        <div class="auth-chip">Users · Keys · Session</div>
      </div>
    </div>
  </div>
</template>
