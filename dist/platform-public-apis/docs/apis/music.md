...
> 本文描述迁移期仍由 Platform legacy Handler 提供的公开契约。API Service 中早期音乐 Route、千千/酷我 Client 和相关配置已经删除，音乐迁移当前暂停；恢复前不会接管本路径。历史设计见 [音乐模块迁移与修复计划](../architecture/v1-music-migration-plan.md)。
